...
public class RMCStation {
    ...
    public int  timeMD;                 // Thời gian Trạm Bơm bê tông tính theo đơn vị khối/phút vào xe
    public ArrayList<Integer> lstD;     // Danh sách khoản cách từ trạm trộn đến các công trường
    ...
    public int calTimeOut(int idSite, int LT, int powerOfTruck){
        ...
        int timeOutStation = timeComeBackStation + ((lstIDT.size() == 0) ? 0 : lstIDT.get(lstIDT.size() - 1).outputTime) + (powerOfTruck * timeMD);
        return timeOutStation;
    }

    public void initListOfIDT(int startStruckID, int powerOfTruck){
        for(int i = 0; i < c; i++){
            ScheduleTruck st = new ScheduleTruck();
            st.truckID = i + startStruckID;
            st.inputTime = 0;
            st.outputTime = powerOfTruck * timeMD * (i + 1) + timeFirstTruckGo;
